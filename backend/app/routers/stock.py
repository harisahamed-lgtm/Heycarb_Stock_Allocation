...
# Excel -> dbo.ZSAPHI
#
# Mapping supplied by "SAP Table column to excel mapping.xlsx":
# Dispatch Month   -> DispatchMonth
# Allocation Month -> AllocationMonth
# Sales Order      -> SalesOrder
# Material Code    -> Grade
# Customer         -> Customer
#
# Other ZSAPHI columns are nullable and remain NULL until
# mappings are supplied for them.
# ============================================================
...
SAP_ORDER_MAPPING = {
    ...
    "Qty": "Qty",
    "Activity": "Packing",
    "Total EX Kiln Req": "TotalEXKilnReq",
    "Yield": "TotalEXKilnReqKG",
}

SAP_ORDER_SHEET = "Capacity Allocation"


def _clean_sap_value(value):
    """Convert pandas values into pyodbc-safe Python values."""
    if pd.isna(value):
        return None
...
async def import_sap_orders(file: UploadFile = File(...)):
    """
    Replace dbo.ZSAPHI with the mapped SAP Orders Excel file.

    DELETE + INSERT is one transaction, so an import failure
    ...
    try:
        ...
        dataframe = pd.read_excel(
            BytesIO(contents),
            sheet_name=SAP_ORDER_SHEET,
            dtype=object,
        )

        missing_columns = [
            excel_column
            for excel_column in SAP_ORDER_MAPPING
        ...
        if missing_columns:
            raise HTTPException(
                ...
                detail={
                    "message": (
                        "The SAP Orders Excel file does not contain "
                        "all required columns from the supplied mapping."
                    ),
                    "missing_columns": missing_columns,
                    "sheet": SAP_ORDER_SHEET,
                },
            )

        dataframe = dataframe[list(SAP_ORDER_MAPPING.keys())].copy()

        dataframe.rename(
            columns=SAP_ORDER_MAPPING,
            inplace=True,
        )

        dataframe = dataframe.dropna(
            how="all",
            subset=list(SAP_ORDER_MAPPING.values()),
        )

        connection = get_connection()
        cursor = connection.cursor()

        # Delete and insert are in the same transaction.
        cursor.execute("DELETE FROM dbo.ZSAPHI")

        insert_sql = """
            INSERT INTO dbo.ZSAPHI
            (
                DispatchMonth,
                AllocationMonth,
                SalesOrder,
        ...
        values = [
            (
                _clean_sap_value(row["DispatchMonth"]),
                _clean_sap_value(row["AllocationMonth"]),
                _clean_sap_value(row["SalesOrder"]),
                _clean_sap_value(row["Grade"]),
                _clean_sap_value(row["Customer"]),
                _clean_sap_value(row["Qty"]),
                _clean_sap_value(row["Packing"]),
                _clean_sap_value(row["TotalEXKilnReq"]),
                _clean_sap_value(row["TotalEXKilnReqKG"]),
                None,  # CumQty is not mapped in the supplied workbook.
            )
            for _, row in dataframe.iterrows()
        ]
        ...
        return {
            "message": (
                "SAP Orders imported successfully. "
                "Existing ZSAPHI records were replaced."
            ),
            "rowsInserted": inserted_rows,
            "mappedColumns": SAP_ORDER_MAPPING,
            "unmappedTableColumns": [
                "CumQty",
            ],
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"SAP Orders import failed: {str(exception)}",
        )      
