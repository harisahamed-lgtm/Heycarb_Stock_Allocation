...
function StockImportExport() {
  ...
  // SAP Orders import: replaces dbo.ZSAPHI through the new backend endpoint.
  const handleSapImport = async () => {
    ...
    const formData = new FormData();
    formData.append("file", selectedSapFile);

    showStatus("Importing SAP Orders...", "info");

    try {
      const response = await fetch(
        "http://localhost:8000/api/stock/import-orders",
        {
          method: "POST",
          body: formData,
        }
      ...
      if (!response.ok) {
        let detail = result.detail || "SAP Orders import failed.";
        ...
        if (typeof detail === "object") {
          ...
          detail = missing
            ? `${detail.message || "Import failed."} Missing columns: ${missing}`
      ...
      setSelectedSapFile(null);

      if (sapFileRef.current) {
        sapFileRef.current.value = "";
      }
    } catch (error) {
      showStatus(
        error.message || "Unable to import SAP Orders.",
        "error"
      );
    }
  };
